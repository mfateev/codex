...
impl Session {
    ...
    pub async fn replace_history(&self, items: Vec<ResponseItem>) {
        let mut state = self.state.lock().await;
...
mod tests {
    ...
    // NOTE: The `MockModelStreamer` and `MockToolCallHandler` tests that
    // exercised `try_run_sampling_request` with `&mut dyn ModelStreamer` were
    // removed during an upstream merge that refactored `ModelClientSession::stream`
    // to accept per-turn parameters. The `ToolCallHandler` trait and its
    // `ToolCallRuntime` impl remain fully functional; end-to-end mock tests will
    // be re-added once a `ModelStreamer` adapter for the new stream API is built.
}
