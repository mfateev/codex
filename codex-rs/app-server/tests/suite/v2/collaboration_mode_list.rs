...
//! includes the plan and default modes with their default model and reasoning effort settings,
//! which keeps the API contract visible in one place.
...
async fn list_collaboration_modes_returns_presets() -> Result<()> {
    ...
    let expected = [plan_preset(), default_preset()];
