...
impl App {
    ...
    async fn handle_event(
        ...
        match event {
            ...
            AppEvent::CodexEvent(_event) => {
                // TODO: Route external AgentSession events through the
                // app-server event pipeline once run_with_session is restored.
                tracing::warn!("CodexEvent received but external session event routing is not yet implemented");
            }
            ...
            AppEvent::SwitchExternalSession(id) => {
                // TODO: Re-implement for app-server architecture.
                // The external session switching needs to bridge to the
                // app-server protocol once run_with_session is restored.
                tracing::warn!(
                    session_id = %id,
                    "SwitchExternalSession not yet implemented for app-server architecture"
                );
            }
    ...
    async fn handle_thread_created(&mut self, thread_id: ThreadId) -> Result<()> {
        ...
        // This method is used by the external-session (Temporal) path where
        // a ThreadManager-backed AgentSession provides thread handles.
        // In app-server mode the server manages thread subscriptions directly,
        // so this is a no-op stub that compiles but is never called.
        tracing::warn!(
            "handle_thread_created called for {thread_id} but no ThreadManager is available"
        );
        Ok(())
    }
